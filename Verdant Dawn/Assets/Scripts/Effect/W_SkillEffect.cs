...
public class W_SkillEffect : RecycleObject
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (isDamage && other.gameObject.CompareTag("Enemy"))
        {
            ...
            if (isContinuous)
            {
                wSkillCoroutine = ContinuousDamage(other);
                StartCoroutine(wSkillCoroutine);
            }
    ...
    {
        if (wSkillCoroutine != null && isContinuous)
        {
            StopCoroutine(wSkillCoroutine);
        }
    }
