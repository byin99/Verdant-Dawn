...
public class EnemyDeath : EnemyBase
{
    ...
    IEnumerator Dissolve(EnemyController sender)
    {
        ...
        while (timeElapsed < dissolveDuration)              // 시간 될때까지 반복
        {
            ...
            for (int i = 0; i < sender.materials.Length; i++)
            {
                sender.materials[i].SetFloat(Fade_ID, 1 - (timeElapsed * fadeNormalize));  // fade값을 1 -> 0으로 점점 감소시키기
            }
        ...
        for (int i = 0; i < sender.materials.Length; i++)
        {
            sender.materials[i].SetFloat(Fade_ID, 0);
        }
