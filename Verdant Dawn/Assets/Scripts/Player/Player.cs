...
public class Player : MonoBehaviour
{
    ...
    /// </summary>
    public bool CanAttack => (!movement.isRoll && !state.isChange && !attack.isUseSkill);

    /// <summary>
    /// 움직일 수 있음을 알리는 프로퍼티(true면 움직일 수 있음)
    /// </summary>
    public bool CanMove => (!movement.isRoll && !attack.isAttack && !attack.isUseSkill);

    /// <summary>
    /// Class를 바꿀 수 있음을 알리는 프로퍼티(true면 바꿀 수 있음)
    /// </summary>
    public bool CanChange => (!attack.isAttack && !attack.isUseSkill);

    /// <summary>
    /// 스킬을 사용할 수 있음을 알리는 프로퍼티(true면 사용할 수 있음)
    /// </summary>
    public bool CanUseSkill => (!attack.isAttack && !movement.isRoll && !state.isChange && !attack.isUseSkill);
    ...
    private void Awake()
    {
        ...
        state = GetComponent<PlayerClass>();
